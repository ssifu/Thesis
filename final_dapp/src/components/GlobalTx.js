...
const RecentTx = () => {
  ...
  useEffect(() => {
    async function getData() {
      const tx = await App.transactionContract.filters.transactions();
      const txData = await App.transactionContract.queryFilter(tx);
      setData(txData);
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center p-3 text-white">
      {data.map((e) => {
        const myDate = new Date(e.args.date.toString() * 1000);
        return (
          <div
